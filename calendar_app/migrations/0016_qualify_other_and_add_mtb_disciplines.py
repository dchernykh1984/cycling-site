...
# Trailing " (...)" suffix appended by the forward rename, stripped on reverse.
...
def strip_direction_suffix(apps, schema_editor):
    """Reverse: drop the trailing " (direction)" added above, restoring the bare generic name."""
    Discipline = apps.get_model("calendar_app", "Discipline")
    for d in Discipline.objects.filter(name_en__startswith=f"{_GENERIC_NAME_EN} ("):
        for loc in ("ru", "kk", "en"):
            base = getattr(d, f"name_{loc}")
            if base:
                setattr(d, f"name_{loc}", _SUFFIX_RE.sub("", base))
        if d.name:
            d.name = _SUFFIX_RE.sub("", d.name)
        d.save(update_fields=["name", "name_ru", "name_kk", "name_en"])
...
def remove_mtb_disciplines(apps, schema_editor):
    """Reverse: delete the two Mountain Bike formats added above."""
    Discipline = apps.get_model("calendar_app", "Discipline")
    Discipline.objects.filter(category__name_en="Mountain Bike", name_en__in=_NEW_MTB_NAMES_EN).delete()
