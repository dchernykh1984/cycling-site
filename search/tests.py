...
class SearchViewTests(TestCase):
    ...
    def test_search_with_query(self):
        self.assertEqual(self.client.get("/search/", {"query": "Home"}).status_code, 200)

    def test_search_with_non_integer_page(self):
        self.assertEqual(self.client.get("/search/", {"query": "Home", "page": "not-a-number"}).status_code, 200)

    def test_search_with_out_of_range_page(self):
        self.assertEqual(self.client.get("/search/", {"query": "Home", "page": 9999}).status_code, 200)
    ...
    def test_visible_article_found(self):
        ...
        resp = self.client.get("/search/", {"query": "VisibleGuide"}, HTTP_ACCEPT_LANGUAGE="ru")
    ...
    def test_hidden_article_excluded_from_search(self):
        ...
        get_search_backend().add(art)
        resp = self.client.get("/search/", {"query": "HiddenSecret"}, HTTP_ACCEPT_LANGUAGE="ru")
        titles = [a.title for a in resp.context["knowledge_results"]]
    ...
    def test_deleted_article_excluded_from_search(self):
        ...
        get_search_backend().add(art)
        resp = self.client.get("/search/", {"query": "DeletedSecret"}, HTTP_ACCEPT_LANGUAGE="ru")
        titles = [a.title for a in resp.context["knowledge_results"]]
