from agent.dedup import is_duplicate, title_tokens
...
def test_verbose_retitling_same_date_is_duplicate():
    ...
    assert is_duplicate("Apricot Marathon Gravel MTB Race 2026", "2026-08-09", known) is True
...
def test_extra_words_but_shared_core_same_date_is_duplicate():
    ...
    assert is_duplicate(cand, "2026-08-15", known) is True


def test_only_generic_words_shared_is_not_duplicate():
    # Mirrors "Road Race ..." vs a very different "Road Race autumn ITT MTB ..." -- too little overlap.
    known = [_known("Road Race autumn ITT road MTB run 10 km duathlon", "2026-09-20")]
    assert is_duplicate("Road Race highway ride relay", "2026-09-20", known) is False


def test_single_shared_word_is_not_duplicate():
    known = [_known("Almaty Criterium", "2026-07-01")]
    assert is_duplicate("Almaty Gran Fondo", "2026-07-01", known) is False  # only "almaty" in common


def test_same_name_different_year_is_not_duplicate():
    known = [_known("Apricot Marathon Gravel MTB 2025", "2025-08-09")]
    assert is_duplicate("Apricot Marathon Gravel MTB 2026", "2026-08-09", known) is False  # dates far apart


def test_close_date_within_window_still_matches():
    known = [_known("Apricot Marathon Gravel MTB", "2026-08-09")]
    assert is_duplicate("Apricot Marathon Gravel MTB Race", "2026-08-12", known) is True  # 3 days apart


def test_too_few_meaningful_words_is_not_duplicate():
    known = [_known("Road Race extra words here", "2026-09-20")]
    assert is_duplicate("Race", "2026-09-20", known) is False  # candidate has < 2 meaningful tokens
